...
- **Per-model overrides can only be set for the current route + keys that already exist**: the panel deliberately does not fetch a provider/model directory (that would add an unauthenticated round trip), so the override card lists the currently resolved route (`provider` and `provider/model`) plus the override keys already in the table. **To pre-seed a rule for another provider/model you must switch to that model first**; keys you have set once stay listed and remain editable. If the middle layer fails to mount, the runtime table is cleared, but those keys are still rendered from the union of the runtime and persisted (`autoManageByRoutePersisted`) tables and flagged as "saved, not in effect" — still visible, still deletable.
