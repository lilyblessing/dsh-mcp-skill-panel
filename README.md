...
| 版本  | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0.5.3 | 发布批次（2026-08-27）：① 新功能（承接 PR #5）：MCP 快速迁移添加（粘贴 mcpServers JSON → 全局/项目）、项目级工作空间 MCP（.dsh/mcps 热更新 + 按会话隔离）、工具级禁用（mcp_search 过滤 + mcp_call 拒绝）、创建技能（全局/项目）② 测试期修复：/config GET 免鉴权（生效时机显示）；scope key 进程级共享缓存（修复面板 live server 聚合无工具）；工具级禁用 UI 全 server 可用（catalog 兜底）；setRowFlag 反转 + 外部修改保守化（obsidian 设置丢失事故）③ 工程：装配过滤空表快速通道（177x+）；CI 适配分支保护（App token 回写 Bypassed）；发布前独立审查整改（status 以真实注册判定） |
