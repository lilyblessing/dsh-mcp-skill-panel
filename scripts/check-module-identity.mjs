...
/** 宿主基准候选：显式 → 本包上层含 @deepseek-ai 的 node_modules → NODE_PATH 影子树。 */
function hostBases(pluginBasis) {
  const out = []
  const explicit = (process.env.DSH_MODULE_BASIS ?? '').trim()
  if (explicit.length > 0) out.push(explicit)
  let dir = pluginBasis
  for (let i = 0; i < 6; i += 1) {
...
if (failed) {
  ...
  console.error('      验装机请带两个基准环境变量：')
  console.error("        $env:PLUGIN_BASIS='…\\profiles\\web\\node_modules\\dsh-mcp-skill-panel'")
  console.error("        $env:DSH_MODULE_BASIS='…\\profiles\\node_modules'")
  process.exit(1)
}
