...
import { n as readState, s as writeState } from "./state-Bqe411EF.mjs";
...
function registerMcpSearchTool(ctx, control, controller) {
	const definition = defineTool({
		...
		description: `检索可用的 MCP 服务器与工具目录（只读，不执行）。四种用法：① 空参数 → server 清单（含已关闭的，标注开/关）；② server=X → 该 server 的**能力摘要**（工具总数 + 前 5 个名字预览，不返回全表，避免上下文膨胀）；③ query + server → 在 X 内按需检索，返回 top-K 命中（含完整 schema），**想找某个 server 上的具体工具就用这个**；④ query → 全目录关键词检索。查到工具名后用 ${MCP_CALL_TOOL}(server, tool, arguments) 调用；不知道工具名先用 ②/③，不要用 ② 拉全表（工具多时传 all:true 才会返回全表）。中文连写请用空格分词（如“搜索 网页”）。`,
		parameters: {
			query: {
				type: "string",
		...
		execute: async (args, exec) => {
			...
			return toJson({
				ok: true,
				kind: "summary",
				summary: [`已安装 ${servers.length} 个 MCP server（${openCount} 个已打开并对模型可见，${servers.length - openCount} 个已关闭——关闭的对模型不可见，但可经 ${MCP_CALL_TOOL} 按需临时拉起）。`, ...servers.map((s) => `- ${s.server} [${s.open ? "开" : "关"}]${s.tools === null ? "" : ` (${s.tools} 工具)`}: ${s.summary}`)].join("\n"),
				servers,
...
//#endregion
export { snapshotFromSchemas as A, rebuildOwnersFromState as C, loadCatalog as D, messageOf as E, saveCatalog as O, projectServerOwner as S, scanWorkspaceMcp as T, setToolDisabled as _, gatewayCall as a, installProjectMcp as b, mcpcall_exports as c, normalizeToolName as d, disabledToolsOf as f, resolveToolBulkTargets as g, loadDisabledTools as h, createMcpCallController as i, serverOfMcp as k, msgOf as l, isToolDisabled as m, MCP_CALL_TOOL as n, installMcpControlTools as o, installToolDisableFilter as p, MCP_SEARCH_TOOL as r, inventoryTraceDiag as s, CONTROL_TOOL_NAMES as t, normalizeArguments as u, setToolsDisabledBulk as v, remountWorkspace as w, projectServerName as x, getActiveWorkspace as y };
