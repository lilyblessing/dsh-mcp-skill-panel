	...
	factory: (require) => {
		...
		*/
		...
		const en = {
			...
			"ri.statusUnregisteredHint": "The row is enabled and running but has registered 0 tools — the subprocess likely failed to start or is idling (e.g. a missing project index). Tools still listed below come from the cached catalog; mcp_call will retry once before failing.",
			...
			"ri.aiOwnedHint": "Temporarily enabled by the model via mcp_call (AI middle layer). Stays invisible to the model and is auto-disabled after ~30s idle — unlike you opening it yourself: no cache cost, no permanent tool prefix.",
		...
		const zh = {
			...
			"ri.statusUnregisteredHint": "该行已启用且在运行，但**零工具注册** —— 子进程多半没起来或空转（如缺项目索引）。下方列出的工具来自目录快照，不代表当前可用；mcp_call 会重试一次后失败。",
			"ri.toolsCount": "{n} 个工具",
			...
			"ri.aiOwnedHint": "该行由模型经 mcp_call 临时启用（AI 中间层）。它对模型保持不可见，空闲约 30s 后自动关闭——与你手动打开不同：不占上下文、不改每轮工具前缀、无缓存代价。",
