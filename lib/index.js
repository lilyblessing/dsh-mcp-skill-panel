...
* - 否则 'mount'。
*/
function decideMount(serverName, presetConfig, presetDisabled, mounted) {
...
async function toggleMcp(deps, entryId, disabled, applyMode) {
	...
	if (!entry) {
		...
		if (found) {
			...
			try {
				const { rowDisabledState } = await Promise.resolve().then(() => preset_exports);
...
//#endregion
export { Config, apply, applyPendingMcp, buildSkillMd, checkChildVisible, computeStatus, createGatewayState, decideMount, disabledToolsOf, disposeGatewayState, findPresetRowByServerName, gatewayCall, inject, installProjectMcp, isToolDisabled, isValidSkillName, isolateChildScope, loadDisabledTools, mergeSchemas, msgOf, name, normalizeArguments, normalizeToolName, parsePresetMcpText, pendingMcp, pendingMcpCount, presetConfigOf, projectServerName, projectServerOwner, readState, remountWorkspace, rowDisabledState, scanWorkspaceMcp, setRowFlag, setSkillFlag, setToolDisabled, syncPresetFiles, writeState };
