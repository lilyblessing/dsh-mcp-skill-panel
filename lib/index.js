...
async function syncRows(ctx, state, rows) {
	...
	for (const [serverName, row] of wanted) {
		...
		if (otherOwner !== void 0 && otherOwner !== state.root) {
			ctx.logger.warn?.(`mcp-skill-panel: 项目 MCP "${serverName}"（${state.root}）与工作空间 ${otherOwner} 重名，已跳过（serverName 全进程唯一）`);
			continue;
		}
		try {
...
async function applyStateResidue(deps, state) {
	...
	for (const [file, entries] of byFile) {
		...
		for (const { entry, rowState } of entries) {
			if (rowDisabledState(text, entry.options.id) !== rowState.lastApplied) {
...
async function appendGlobalPatch(ctx, yamlBlock) {
...
async function writeProjectMcp(workspace, servers) {
