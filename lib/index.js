...
function normalizeToolName(serverName, toolName) {
	const prefix = `mcp__${serverName}__`;
	let name$1 = toolName;
	if (name$1.startsWith("mcp__")) {
		while (name$1.startsWith(prefix)) name$1 = name$1.slice(prefix.length);
		if (name$1.startsWith("mcp__")) throw new Error(`mcp_call: tool 参数疑似其他 MCP server 的注册全名（${JSON.stringify(toolName)}，server="${serverName}"）；请传该 server 上的裸名（如 understand_image，不带 mcp__ 前缀）`);
	}
	return name$1;
}
/**
...
function collectToolViews(ctx, agent) {
	...
	const agentTools = (() => {
		try {
			return agent && agent.ctx ? agent.ctx.tools : void 0;
		} catch {
			return void 0;
		}
	})();
	const agentScope = agent ? scopeOf(agent.ctx) : void 0;
	if (agentTools) {
		views.push({
			label: "agent-scoped",
			tools: agentTools,
...
async function waitRegistered(control, ctx, name$1, views, timeoutMs, signal) {
	...
	return new Promise((resolve, reject) => {
		...
		const check = () => {
			...
			for (const view of views) {
				...
				try {
					if (Boolean(view.tools.get(name$1, view.scope))) {
						ctx.logger.info?.(`mcp-skill-panel: tool "${name$1}" resolved via view "${view.label}"`);
	...
	const controller = {
		...
		async call(serverName, toolName, args, agent, signal, explicitTimeoutMs) {
			...
			try {
				...
				const view = await waitRegistered(caches, ctx, name$1, views, timeoutMs, signal);
...
//#endregion
export { Config, apply, applyPendingMcp, inject, msgOf, name, normalizeArguments, normalizeToolName, pendingMcp, pendingMcpCount, readState, rowDisabledState, setRowFlag, setSkillFlag, syncPresetFiles, writeState };