...
import { a as rowDisabledState, c as syncPresetFiles, n as buildSkillMd, o as setRowFlag, r as isValidSkillName, s as setSkillFlag, t as EDITABLE_CONFIG_KEYS } from "./preset-rx_ovWEr.mjs";
...
async function toggleMcp(deps, entryId, disabled, applyMode) {
	...
	if (!entry || gatewayServerOfEntryId(entryId) !== null) {
		...
		if (found) {
			...
			try {
				const { rowDisabledState } = await import("./preset-rx_ovWEr.mjs").then((n) => n.i);
		...
		},
		{
			...
			handler: handleAny([{
				...
				run: async (req) => {
					...
					const nextConfig = { ...described.config ?? {} };
					...
					return {
						...
						server,
						applied,
						after: await describeRow(server)
					};
				}
			}])
		},
		{
			...
			handler: handleAny([{
				...
				run: async (req) => {
					...
					const next = { ...entry.options.config ?? {} };
