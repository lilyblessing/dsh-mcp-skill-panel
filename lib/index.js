import { a as setRowFlag, c as clearStateAiOwner, d as stateApplyMode, f as writeState, i as rowDisabledState, l as readState, n as isValidSkillName, o as setSkillFlag, s as syncPresetFiles, t as buildSkillMd, u as setStateAiOwner } from "./preset-CFZudtaS.mjs";
import { a as serversToRows, i as serversToPatchYaml, n as parseMcpServersJson, r as resolveServersEnv } from "./mcp-convert-QL_5hLe8.mjs";
...
async function callViaLoaderEntry(ctx, control, state, serverName, bareTool, name, normArgs, opts, entry) {
	...
	try {
		...
		if (failed && aiOwned && next <= 0) restoreGateway(control, ctx, serverName, entryId);
	}
}
/** loader 分支的 ensureEnabled（不碰 ControllerState.aiEnabled；网关行用户语义恒用户打开）。 */
async function ensureEnabledGateway(control, ctx, serverName, entry) {
...
async function restoreGateway(control, ctx, serverName, entryId) {
	try {
		const entry = control.resolveEntry(serverName);
	...
	try {
		...
		if (!pid) {
			state.lastCheck = {
				at: Date.now(),
				ok: true,
				detail: "mounted=0 reused=0 skipped=0 skippedOfficial=0 errors=0 (no preset)"
			};
			return out;
		}
...
async function toggleMcp(deps, entryId, disabled, applyMode) {
	...
	if (!entry || gatewayServerOfEntryId(entryId) !== null) {
		...
		if (found) {
			...
			try {
				const { rowDisabledState } = await import("./preset-CFZudtaS.mjs").then((n) => n.r);
	...
	const routes = [
		...
		},
		{
			...
			handler: handle("GET", async () => {
				...
				let gateway;
				try {
					const { gatewayStateForDebug } = await import("./index.js");
