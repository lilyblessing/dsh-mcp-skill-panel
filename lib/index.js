...
*/
function schemasOfView(view) {
	if (!view?.tools) return [];
	try {
		return view.tools.schemas?.(view.scope) ?? [];
	} catch {
		return [];
	}
}
...
async function collectInventory(ctx, caches, state, serverName, requestedBy = "unknown") {
	const t0 = Date.now();
	const trace = {
		at: t0,
		requestedBy,
	...
	let out = null;
	try {
		const views = collectToolViews(ctx, void 0);
		const view = await waitRegistered(ctx, `mcp__${serverName}__`, views, caches.serverTimeoutMs(serverName), void 0);
		trace.viewLabel = view?.label ?? null;
		trace.viewScope = view?.scope === void 0 ? "undefined" : typeof view.scope;
		mark("waitRegistered");
		state.lastUsed.set(serverName, Date.now());
		const schemas = schemasOfView(view);
		trace.schemaTotal = schemas.length;
		trace.schemaMatched = schemas.filter((s) => String(s?.name ?? "").startsWith(`mcp__${serverName}__`)).length;
		mark(`schemasOfView(n=${schemas.length}/m=${trace.schemaMatched})`);
		if (schemas.length > 0) {
			out = await caches.storeInventory?.(serverName, schemas) ?? null;
			trace.stored = out?.tools ?? null;
			mark(`storeInventory(n=${out?.tools ?? 0})`);
		}
		if (!out) {
			out = await caches.collectInventory?.(serverName) ?? null;
			trace.stored = out?.tools ?? null;
			mark(`fallbackCollect(n=${out?.tools ?? 0})`);
		}
		if (!out) {
			inventoryFailUntil.set(serverName, Date.now() + INVENTORY_FAIL_TTL_MS);
			stop("empty", `view=${trace.viewLabel} schemas=${schemas.length} matched=${trace.schemaMatched}`);
		}
	} catch (error) {
		inventoryFailUntil.set(serverName, Date.now() + INVENTORY_FAIL_TTL_MS);
...
function registerMcpSearchTool(ctx, control, controller) {
	const definition = defineTool({
		name: "mcp_search",
		description: "检索可用的 MCP 服务器与工具目录。三层：空参数返回 server 清单（无 schema）；传 server 列出该服务器工具（分页，无 schema）；传 query 做关键词 top-K 全文检索（命中返回完整 schema）。知道工具名可直接 mcp_call，不知道先用关键词搜。中文连写请用空格分词（如“搜索 网页”）。",
		parameters: {
			query: {
				type: "string",
				description: "检索关键词，按工具名/描述/参数名打分（缺省 top-K 8，上限 10）"
			},
			server: {
				type: "string",
				description: "列出指定 MCP server 的全部工具（分页，无 schema）"
			},
			limit: {
				type: "integer",
				description: "关键词 top-K（默认 8）或 server 页大小（默认 20，上限 50）"
			},
			offset: {
				type: "integer",
				description: "server 页偏移（默认 0，仅 server 分支有效）"
			},
			topK: {
				type: "integer",
		...
		execute: async (args, exec) => {
			...
			if (server) {
				...
				return toJson({
					...
					limit: pageLimit,
					tools,
					hint: page.hasSnapshot ? "工具多时改 query + server 缩小范围；中文连写请用空格分词。" : `该 server 已安装但当前关闭且采集失败（无工具快照）。可直接 mcp_call 调用它——中间层会临时拉起；若持续失败请在面板打开它。`
				});
			}
			if (query) {
...
async function toggleMcp(deps, entryId, disabled, applyMode) {
	...
	if (deferred) {
		...
		pendingMcp.delete(entryId);
		if (disabled) try {
			await deps.controller?.fetchInventory(serverNameOf(entry));
		} catch (error) {
