...
function installMcpVisibilityFilter(ctx, isMcpVisible) {
	return ctx.effect(() => {
		const off = ctx.root.on("system-prompt/assemble", (assembly, _context, next) => {
			if (assembly && Array.isArray(assembly.tools)) assembly.tools = assembly.tools.filter((tool) => {
...
async function resolveScopeSchemas(ctx) {
	...
	return ctx.tools.schemas(scopeKey);
}
...
function makeRoutes(ctx, caches, catalogRuntime, config = {}, controller) {
	const deps = {
		ctx,
		caches,
		catalogRuntime,
	...
	const routes = [
		...
		},
		{
			...
			handler: (req, res) => {
				...
				snapshotEnabled(ctx, catalogRuntime).then(() => json(res, 200, {
					ok: true,
					diag: catalogRuntime.diag
